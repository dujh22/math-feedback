...
def _get_reward_model(base_pretrained_model, base_llm_model):
    class LLMForSequenceRegression(base_pretrained_model):
        ...
        def forward(
            ...
            values = self.sigmoid(values)  # 应用 Sigmoid 函数
            # # 应用分类头进行二分类
            # values = self.classifier(last_hidden_states).squeeze(-1)
            
            # 找到 input_ids 中等于 128255 和 128001 的索引
            indices_128255 = (input_ids == 128255).nonzero(as_tuple=True)[1]
            indices_128001 = (input_ids == 128009).nonzero(as_tuple=True)[1]

            # 提取相应的 reward 值
            rewards_128255 = values[0, indices_128255]
            rewards_128001 = values[0, indices_128001]

            # 拼接为一个最终的 reward
            reward = torch.cat((rewards_128255, rewards_128001))
