...
def prm_evaluation_best_of_n(id = 2, max_workers_num = 10, maxn = 5, data_length = 5,generate_backbone = "tgi", generate_url = TGI_URL, critic_backbone = "tgi", critic_url = CRITIC_URL):
    ...
    # 1. Generate n responses & referenceAnswer & referenceAnswerValue
    for item in tqdm(data, desc='Generating N Responses'):
        item['responses'] = []
        if generate_backbone == 'mistral7b':
            for i in tqdm(range(maxn), desc="Generating responses with mistral7b"):
                item['responses'].append(get_mistral_response(item['question']))
        else:
            with ThreadPoolExecutor(max_workers=max_workers_num) as executor:
                futures = {executor.submit(llm_response, QUERY_TEMPLATE.format(Question = item['question']), generate_backbone, generate_url): i for i in N}            
                for future in as_completed(futures):
                    response = future.result()
                    item['responses'].append(response)
        
    # 2. Calculate the PRM value & llm_answer_flag & llm_response_flag
    # 保存结果
    id = 0
    ...
    # 3. Calculate the accuracy with referenceAnswer & referenceAnswerValue
...
def main():
    ...
    parser.add_argument('--id', type=str, default="4", help='ID of the model')
    ...
    parser.add_argument('--data_length', type=int, default=10, help='Length of the data')
